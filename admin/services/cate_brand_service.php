...
include("../managers/category_brand_manager.php");
...
function ListItem(){
	...
	if($dataset->num_rows===0){
		...
		while($row = $dataset->fetch_object()){
			...
			$result .="<td><button class='btn btn-warning' data-id='".$row->id."' data-item='services/cate_brand_service.php?type=item' data-page='cate_brand_edit.html' data-title='Modify' onclick='page.modify(this);' ><span class='glyphicon glyphicon-pencil'></span> แก้ไข</button> ";
			$result .="<button class='btn btn-danger' data-id='".$row->id."' data-item='services/cate_brand_service.php?type=item' data-page='cate_brand_del.html' data-title='Remove' onclick='page.remove(this);'><span class='glyphicon glyphicon-trash'></span> ลบ</button></td>";
			$result .= "</tr>";

...
class News_Manager{
	...
	function get_item($id){
		try{
			$sql = "select news.*,t.title as type_name from news ";
			$sql .= "left join type_news t on news.type_new = t.id ";
			$sql .=" where news.id='$id' ;" ; 

			log_warning("news > get item > " . $sql);
			
			$result = $this->mysql->execute($sql);
			
			return $result;
		}catch(Exception $e){
			log_debug("news > get item > error > " . $e->getMessage());
		}
	}
	...
	function list_item(){
		try{
			...
			$sql = "select news.*,t.title type_name from news ";
			$sql .= "left join type_news t on news.type_new = t.id ";
			
			log_warning("news > get list > " . $sql);
			
			$result = $this->mysql->execute($sql);
			
			return $result;
		}catch(Exception $e){
			log_debug("news > get list > error > " . $e->getMessage());
		}
	}
